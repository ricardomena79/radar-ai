...
Nota de portado (2026-08-05): módulo original construido en una rama
paralela que además extendía `Quote` con campos de trazabilidad de precio
(`price_type`, `source`, `market_state`, `price_regular`/`premarket`/
`afterhours`/`overnight`) para un "Data Fusion Engine" que nunca se
integró a Atlas Core. Ese Core sigue congelado -- `Quote` no se tocó acá.
Los `metrics` de abajo leen esos campos con `getattr(..., None)`: quedan
en `None` (puramente informativos, no participan en ningún filtro ni en
el puntaje) en vez de requerir una extensión de Core no aprobada.
"""
...
def evaluate(
    ...
    metrics: Dict[str, Any] = {
        ...
        # Trazabilidad de precio: solo informativo, ver nota de portado
        # al inicio del archivo. `None` si Quote no trae el campo.
        "price_type": getattr(quote, "price_type", None),
        "source": getattr(quote, "source", None),
        "market_state": getattr(quote, "market_state", None),
        "price_regular": getattr(quote, "price_regular", None),
        "price_premarket": getattr(quote, "price_premarket", None),
        "price_afterhours": getattr(quote, "price_afterhours", None),
        "price_overnight": getattr(quote, "price_overnight", None),
