"""Tests de `GET /api/admin/data-dir-full-inventory` (2026-09-03,
...
from atlas_live import data_dir_full_inventory as ddfi  # noqa: E402
...
def test_sin_token_rechaza():
    ...
    try:
        r = _client().get("/api/admin/data-dir-full-inventory")
...
def test_con_token_delega_en_full_report(monkeypatch):
    ...
    monkeypatch.setattr(ddfi, "full_report", lambda: {"ok": True, "entries": [], "total_files": 0})
    try:
        r = _client().get("/api/admin/data-dir-full-inventory?token=secreto-real")
...
def test_ok_false_devuelve_500(monkeypatch):
    os.environ["ATLAS_ADMIN_TOKEN"] = "secreto-real"
    monkeypatch.setattr(ddfi, "full_report", lambda: {"ok": False, "error": "fallo"})
    try:
        r = _client().get("/api/admin/data-dir-full-inventory?token=secreto-real")
