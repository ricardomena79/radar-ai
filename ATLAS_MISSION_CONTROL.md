...
> **⚠️ Banner de estado (Hito 5, Fase 5.4, 2026-09-04, verificación honesta contra el código real -- no una reescritura de este documento).**
> Este documento sigue **desactualizado como fuente de "qué hizo Atlas"**: describe un diseño puntual de 2026-08-02 (una librería de heartbeat + Timeline SQLite para instrumentar *procesos de validación/backtest*), y **NADA de ESE diseño específico está implementado** -- eso sigue siendo cierto. Pero desde entonces Atlas construyó un sistema de aprendizaje-seguro completo y cerrado que este documento no menciona en absoluto: **Hito 3 (Fases 3.0-3.6: snapshot de conocimiento, elegibilidad, observación shadow, activación controlada, evaluación continua y revocación)**, auditado, cerrado, **commiteado y pusheado a esta rama (HEAD real del repositorio)**, y **Hito 4 (Fases 4.1-4.4: test de integración extremo a extremo, autodiagnóstico de integridad, resumen de seguridad público, panel "Seguridad del Aprendizaje" en la Cabina)**, también auditado y cerrado pero **todavía solo local -- sin commit/push/deploy** (a diferencia de Hito 3). **Aclaración importante de nombres**: el panel "⚙ Mission Control" que existe HOY en la Cabina (`atlas_live/static/cabina/index.html`, sección `view-mission-control`) es una cosa completamente distinta de lo que describe este documento -- solo muestra el historial de sesión de mercado (premarket/regular/afterhours), no el sistema de heartbeat/Timeline/Supervisión Inteligente diseñado abajo. **Fuente de verdad real y actual**: el código en sí, y `.claude/plans/ethereal-mixing-anchor.md` (histórico completo de decisiones, sesión por sesión). El resto de este documento se deja sin tocar -- sigue siendo un registro de diseño válido para SU propio alcance específico, solo que nunca se retomó.
